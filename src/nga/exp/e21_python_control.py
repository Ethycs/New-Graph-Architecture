"""E21 - Torch-native end-to-end gradient training of the Typed Protocol
Network on the python_control subset (Phase 18 Track 2).

E21 is the structural mirror of E18, swapping the python_big dataset
(arithmetic + assignment + calls + defs) for ``dataset_python_control``
which ALSO covers single-line control flow: ``if cond: body``,
``while cond: body``, and the optional ``else: body`` clause that may
follow an if-body.

Phase 18 Track 2 architectural prediction: σ_uplift should swing back
positive on python_control because the grammar adds a NEW
branching-ambiguity site that did not exist in python_big -- namely the
``S0_after_if_body`` state, where after the if-body's NEWLINE the parser
truly cannot tell whether the next token will be ``else`` (continue the
if-stmt) or any of the START-token alternatives (start a new
top-level stmt). BOTH continuations are FSM-legal and BOTH are real
Python; this is exactly the kind of structure σ should fire on.

Pipeline (identical to E17 / E18 / E14):

  Phase A -- Classical seed (substrate-independent):
    expected_counts_observed + bayesian_m_step_beta seed posterior.

  Phase B -- End-to-end torch training on the train split.

  Phase C -- Held-out evaluation with σ + energy + margin diagnostics.

In addition E21 BAKES IN compute-efficiency tracking from the start
(per the E20 pattern): per-phase wall-clock, total wall-clock,
inference throughput on the test loop, and peak memory delta.

Headline metrics (E21-specific):
  ...
  * mean_sigma_at_operator_boundary, mean_sigma_at_non_boundary,
    sigma_boundary_ratio.  Boundary set is the UNION of:
      - S0_after_name_at_start (assignment-vs-expression),
      - S{d}_after_name_in_factor (call-vs-variable, all depths),
      - S0_after_if_body (THE NEW branching-ambiguity site).
  * mean_program_depth.
  * Compute efficiency: phase_a_wall_clock_seconds,
    phase_b_wall_clock_seconds, total_wall_clock_seconds,
    inference_throughput_samples_per_sec, peak_memory_kb.
"""
...
try:  # torch is required for E21.
    import torch
except ImportError as _exc:  # pragma: no cover - exercised only on torch-less envs.
    raise ImportError(
        "torch is required for E21; install via `pixi add pytorch` "
        "(or `pip install torch`)."
    ) from _exc


__all__ = ["E21Result", "run_e21"]


READOUT_TYPE_ID = "default"

# Inherited from python_big: the leading-NAME state (assignment-vs-expr).
PYTHON_CONTROL_LEADING_NAME_STATE = "S0_after_name_at_start"

# Inherited from python_big: the call-vs-variable ambiguity at every depth.
_FACTOR_NAME_AMBIGUITY_SUBSTRING = "after_name_in_factor"

# NEW for E21: the if/else branching-ambiguity site. After the if-body's
# NEWLINE, the next token may be 'else' (continue the if-stmt) or a fresh
# top-level dispatch (skip 'else'). This is the LOAD-BEARING addition.
PYTHON_CONTROL_IF_BODY_STATE = "S0_after_if_body"


def _is_structural_ambiguity_state(vertex_id: str) -> bool:
    """Return True iff ``vertex_id`` is a structural-ambiguity state.

    The python_control structural-ambiguity union is:

      * S0_after_name_at_start    -- assignment-vs-expression
      * S{d}_after_name_in_factor -- call-vs-variable (every depth)
      * S0_after_if_body          -- ELSE-vs-fresh-stmt (NEW)

    All three are FSM-deterministic on a given token but the legal
    continuations come from genuinely different productions.
    """
    if vertex_id == PYTHON_CONTROL_LEADING_NAME_STATE:
        return True
    if vertex_id == PYTHON_CONTROL_IF_BODY_STATE:
        return True
...
@dataclass
class E21Result:
    """Aggregate metrics returned by run_e21."""
    ...
    n_train: int
    n_test: int
    # Compute-efficiency metrics (Phase 18 Track 1 pattern).
...
def _empirical_dist_from_window(
    ...
    """Laplace-smoothed categorical from a sliding window of next states."""
...
def run_e21(
    *,
    ...
    """Run torch-native end-to-end gradient training on python_control.

    Writes the standard four artefacts plus decision_trace.jsonl on the
    HELD-OUT test split. Returns aggregate metrics including compute
    efficiency rows (phase_a_wall_clock_seconds,
    phase_b_wall_clock_seconds, total_wall_clock_seconds,
    inference_throughput_samples_per_sec, peak_memory_kb).
    """
    if fsm.vertex_ids[0] != "START":
        raise ValueError(
            f"E21 expects fsm.vertex_ids[0] == 'START'; "
            f"got {fsm.vertex_ids[0]!r}"
        )
    if fsm.vertex_ids[-1] != "ACCEPT":
        raise ValueError(
            f"E21 expects fsm.vertex_ids[-1] == 'ACCEPT'; "
            f"got {fsm.vertex_ids[-1]!r}"
        )
    if PYTHON_CONTROL_LEADING_NAME_STATE not in fsm.vertex_ids:
        raise ValueError(
            f"E21 expects fsm to contain "
            f"{PYTHON_CONTROL_LEADING_NAME_STATE!r}"
        )
    if PYTHON_CONTROL_IF_BODY_STATE not in fsm.vertex_ids:
        raise ValueError(
            f"E21 expects fsm to contain "
            f"{PYTHON_CONTROL_IF_BODY_STATE!r} (the new branching-"
            f"ambiguity site for if/else)"
        )
    if not any(
        _FACTOR_NAME_AMBIGUITY_SUBSTRING in vid for vid in fsm.vertex_ids
    ):
        raise ValueError(
            "E21 expects at least one S{d}_after_name_in_factor vertex"
        )
    ...
    # ------------------------------------------------------------------
    # Step 1 -- Dataset (python_control walker, 22-dim one-hot tokens).
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Step 3 -- Build the torch substrate (counted in Phase B).
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Step 6 -- Phase C: held-out evaluation (timed for throughput).
    # ------------------------------------------------------------------
    ...
    with (
        ...
        # ----- failure_margin_auroc (q10 strict bar) ----------
        ...
        metric_pairs: list[tuple[str, float]] = [
            ...
            # ----- compute-efficiency rows (Phase 18 Track 1 pattern) -----
