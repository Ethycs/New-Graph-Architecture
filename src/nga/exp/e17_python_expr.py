"""E17 - Torch-native end-to-end gradient training of the Typed
Protocol Network on Python expressions (Wave I).

E17 is the structural mirror of E14, swapping the synthetic ListOps
dataset for the first external published-language benchmark: a subset
of real Python validated by stdlib ``ast.parse`` (see
``nga.exp.dataset_python_expr``). Where E14's structural-ambiguity
point is ``S{d}_after_operand`` (continue-list vs. close), E17's
structural-ambiguity point is ``S0_after_name_at_start`` (assignment
vs. expression-statement) -- the actual Python-grammar analogue of a
real continue-vs.-close decision.

Pipeline (identical to E14):

  Phase A -- Classical seed (substrate-independent):
    Generate the python_expr dataset; on the (current_state,
...
Headline metrics (E17-specific):
  ...
  * mean_sigma_at_operator_boundary (here: at S0_after_name_at_start --
    the assignment vs expression decision point), mean_sigma_at_non_boundary,
...
Acceptance bars are observed-only (mirroring E13/E14): the runner is
NOT tuned to satisfy a strict-uplift threshold; if the test finds the
strict bar is missed, the test's xfail-with-observed-value path
...
try:  # torch is required for E17, but import remains tolerant for clarity.
    import torch
except ImportError as _exc:  # pragma: no cover - exercised only on torch-less envs.
    raise ImportError(
        "torch is required for E17; install via `pixi add pytorch` "
        "(or `pip install torch`)."
    ) from _exc


__all__ = ["E17Result", "run_e17"]


READOUT_TYPE_ID = "default"

# The structural-ambiguity point on the Python-expression FSM. Unlike
# ListOps' S{d}_after_operand (which appears at multiple depths), the
# Python grammar's only genuine continue-vs.-close decision at
# top-level is the assignment-vs.-expression-statement choice after a
# leading NAME; we match the vertex name exactly.
PYTHON_AMBIGUITY_STATE = "S0_after_name_at_start"
...
@dataclass
class E17Result:
    """Aggregate metrics returned by run_e17."""
...
def _max_paren_depth(source: str) -> int:
    ...
    Used as a proxy for parse-tree hierarchy stress (mean across
    programs is reported). Counts only ``(`` / ``)`` since the
    expression subset has no other bracket types.
    """
    depth = 0
    max_d = 0
    for ch in source:
...
def run_e17(
    *,
    ...
    """Run torch-native end-to-end gradient training on Python expressions.
    ...
    plus the Python-grammar structural-ambiguity diagnostics.
    """
    if fsm.vertex_ids[0] != "START":
        raise ValueError(
            f"E17 expects fsm.vertex_ids[0] == 'START'; "
            f"got {fsm.vertex_ids[0]!r}"
        )
    if fsm.vertex_ids[-1] != "ACCEPT":
        raise ValueError(
            f"E17 expects fsm.vertex_ids[-1] == 'ACCEPT'; "
            f"got {fsm.vertex_ids[-1]!r}"
        )
    if PYTHON_AMBIGUITY_STATE not in fsm.vertex_ids:
        raise ValueError(
            f"E17 expects fsm to contain {PYTHON_AMBIGUITY_STATE!r} "
    ...
    # ------------------------------------------------------------------
    # Step 1 -- Dataset (Python expression walker, 16-dim one-hot tokens).
    # ------------------------------------------------------------------
    ...
    # Boundary == the structural-ambiguity vertex on the Python grammar:
    # exactly S0_after_name_at_start (assignment vs expression-statement).
    is_boundary_state = np.array(
        [vid == PYTHON_AMBIGUITY_STATE for vid in vertex_ids],
        dtype=bool,
    )
    ...
    with (
        ...
        # On Python expressions the only genuine top-level continue-vs-
        # close decision is the assignment-vs-expression-statement
        # choice after a leading NAME, i.e. exactly the
        # ``S0_after_name_at_start`` vertex. Match by exact string so
        # we don't accidentally label the depth-encoding S{d}_factor
        # / S{d}_after_term states.
        ambiguity_labels = np.array(
            [
                1 if name == PYTHON_AMBIGUITY_STATE else 0
