"""E18 - Torch-native end-to-end gradient training of the Typed
Protocol Network on the bigger Python subset (Wave II).

E18 is the structural mirror of E17, swapping the python_expr dataset
(arithmetic + assignment only) for ``dataset_python_big`` -- a richer
real-Python subset that ALSO covers function calls, function
definitions, and return statements (see
``nga.exp.dataset_python_big``).

Phase 14's architectural prediction: σ_uplift should be MORE positive
on bigger Python than on python_expr because the grammar has many MORE
structural-ambiguity points. Where python_expr has exactly one top-
level continue-vs.-close decision (``S0_after_name_at_start`` --
assignment vs. expression-statement), python_big has that one PLUS
``S{d}_after_name_in_factor`` at every depth d in {0, 1, 2, 3} (the
NAME-vs.-CALL ambiguity inside a factor). Margin has less headroom on
the richer grammar; σ adds complementary signal exactly there.

Pipeline (identical to E17 / E14):

  Phase A -- Classical seed (substrate-independent):
    Generate the python_big dataset; on the (current_state,
...
Headline metrics (E18-specific):
  ...
  * mean_sigma_at_operator_boundary -- here BOUNDARY means the union of
    every ``S{d}_after_name_in_factor`` (call-vs-variable ambiguity at
    each depth) PLUS ``S0_after_name_at_start`` (assignment-vs-
    expression ambiguity). All other states are non-boundary.
  * mean_program_depth -- max paren nesting per program, proxy for
    hierarchy stress.
  * n_torch_trainable_params, final_loss,
    loss_decrease_first_to_last_epoch.

Acceptance bars are observed-only (mirroring E13/E14/E17): the runner
is NOT tuned to satisfy a strict-uplift threshold; if a test finds
the strict bar is missed, the test's xfail-with-observed-value path
documents the honest result without changing the runner.
"""
...
try:  # torch is required for E18, but import remains tolerant for clarity.
    import torch
except ImportError as _exc:  # pragma: no cover - exercised only on torch-less envs.
    raise ImportError(
        "torch is required for E18; install via `pixi add pytorch` "
        "(or `pip install torch`)."
    ) from _exc


__all__ = ["E18Result", "run_e18"]


READOUT_TYPE_ID = "default"

# Canonical "leading-NAME" ambiguity vertex on the python_big FSM:
# assignment-vs-expression-statement after a top-level NAME. Same
# semantics as the python_expr ambiguity vertex.
PYTHON_BIG_LEADING_NAME_STATE = "S0_after_name_at_start"

# Python-big additionally has the call-vs-variable ambiguity at every
# depth d: the NAME inside a factor may be followed by '(' (CALL) or by
# anything else (bare variable reference). We detect those vertices by
# substring match against ``after_name_in_factor`` so this stays correct
# whatever set of depths the FSM exposes.
_FACTOR_NAME_AMBIGUITY_SUBSTRING = "after_name_in_factor"


def _is_structural_ambiguity_state(vertex_id: str) -> bool:
    """Return True iff ``vertex_id`` is at a structural-ambiguity point.

    Two kinds of ambiguity sit at the same syntactic position on
    python_big:

    * S0_after_name_at_start -- assignment-vs-expression-statement
      ambiguity (continue with ``=`` or with expression operators /
      NEWLINE).
    * S{d}_after_name_in_factor (any d in 0..max_paren_depth) -- the
      call-vs-variable ambiguity inside a factor.

    Both points are FSM-deterministic on a given token but adjacent
    legal continuations come from genuinely different productions; the
    parser cannot decide WHICH continuation is the right one until it
    sees the next token. This is exactly where σ should buy us
    something the margin cannot.
    """
    if vertex_id == PYTHON_BIG_LEADING_NAME_STATE:
        return True
    return _FACTOR_NAME_AMBIGUITY_SUBSTRING in vertex_id
...
@dataclass
class E18Result:
    """Aggregate metrics returned by run_e18."""
...
def _max_paren_depth(source: str) -> int:
    """Return the maximum parenthesis nesting depth in ``source``."""
    depth = 0
    max_d = 0
    for ch in source:
        if ch == "(":
...
def run_e18(
    *,
    ...
    """Run torch-native end-to-end gradient training on bigger Python.
    ...
    plus the python_big multi-ambiguity-point structural diagnostics.
    """
    if fsm.vertex_ids[0] != "START":
        raise ValueError(
            f"E18 expects fsm.vertex_ids[0] == 'START'; "
            f"got {fsm.vertex_ids[0]!r}"
        )
    if fsm.vertex_ids[-1] != "ACCEPT":
        raise ValueError(
            f"E18 expects fsm.vertex_ids[-1] == 'ACCEPT'; "
            f"got {fsm.vertex_ids[-1]!r}"
        )
    if PYTHON_BIG_LEADING_NAME_STATE not in fsm.vertex_ids:
        raise ValueError(
            f"E18 expects fsm to contain {PYTHON_BIG_LEADING_NAME_STATE!r} "
            f"(the assignment-vs-expression decision point); "
            f"got vertex_ids={fsm.vertex_ids!r}"
        )
    # Sanity: at least one call-vs-variable ambiguity vertex exists.
    if not any(
        _FACTOR_NAME_AMBIGUITY_SUBSTRING in vid for vid in fsm.vertex_ids
    ):
        raise ValueError(
            f"E18 expects fsm to contain at least one "
            f"S{{d}}_after_name_in_factor vertex; got "
    ...
    # ------------------------------------------------------------------
    # Step 1 -- Dataset (python_big walker, 18-dim one-hot tokens).
    # ------------------------------------------------------------------
    ...
    # Boundary == any python_big structural-ambiguity vertex (union of
    # the leading-NAME assignment-vs-expr point and every depth's
    # call-vs-variable point).
    is_boundary_state = np.array(
        [_is_structural_ambiguity_state(vid) for vid in vertex_ids],
        dtype=bool,
    )

    # Mean program depth (max paren nesting per program), reported as a
    # proxy for parse-tree hierarchy stress.
    if ds.programs:
        mean_program_depth = float(
            np.mean([_max_paren_depth(s) for s in ds.programs])
        )
    else:
    ...
    with (
        ...
        # On python_big the structural-ambiguity points are the union
        # of S0_after_name_at_start (assignment-vs-expression) AND every
        # S{d}_after_name_in_factor (call-vs-variable). We label each
        # test step by whether ITS current_state is in that union.
