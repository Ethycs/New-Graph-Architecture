"""E14 - Torch-native end-to-end gradient training of the Typed
Protocol Network on ListOps (Phase 9).

Where E13 ran the classical Phase A + KL-blended Phase B framework on
ListOps with an sklearn substrate, E14 swaps the substrate for the
torch end-to-end gradient trainer
(:class:`nga.arch.torch_energy_trainer.TorchEnergyTrainer`) and runs
genuine gradient training on the prototype tower, the posterior log-
counts, and the typed-readout heads through a single Adam optimiser.

The encoder (:class:`FrozenEncoderTorch`) remains frozen by
construction so the substrate remains "frozen encoder + learnable
prototype/readout/posterior."
...
Pipeline:
  ...
  Phase A -- Classical seed (substrate-independent):
    ...
    posterior_beta_init. The Phase-A Hamming distance from the gold
    legality matrix is reported BEFORE any torch update touches the
    posterior, so it is by construction substrate-independent.

  Phase B -- End-to-end torch training:
    Run K epochs over the train split. Each step:
      * encode the observation through FrozenEncoderTorch;
      * (optionally) read the readout's logits to provide a
        substrate-coherent confidence channel for tagging/control;
      * compute the trainer's forward (energy + KL term);
      * loss.backward(); torch.nn.utils.clip_grad_norm_;
      * optimizer.step().
    The Adam optimiser is the trainer's own (built once in
    TorchEnergyTrainer.__init__ over every requires_grad parameter the
    trainer registers as a child module). This includes the readout
    heads -- registering them via nn.ModuleDict in the trainer is the
    one subtlety that lets a single Adam reach prototypes + posterior
    log-counts + readout weights together.

  Phase C -- Held-out evaluation:
    Snapshot the torch posterior to a numpy PosteriorMask (one-way),
    then evaluate on the held-out test split. Per step record sigma,
    energy, margin, kl_surprise; the test-set predictions feed
    failure_margin_auroc to produce sigma_auroc / margin_auroc /
    sigma_uplift -- the Phase 2 q10 strict bar.

Headline metrics (Phase 9-specific):
  * phase_a_hamming_normalised, phase_b_hamming_normalised.
  * sigma_auroc, margin_auroc, sigma_uplift (q10 strict bar).
  * mean_sigma_at_operator_boundary, mean_sigma_at_non_boundary,
    sigma_boundary_ratio.
  * n_torch_trainable_params (from the trainer's parameter list).
  * final_loss, loss_decrease_first_to_last_epoch.

Acceptance bars are intentionally observed-only (mirroring E13). The
runner is NOT tuned to satisfy a strict-uplift threshold; if the test
...
try:  # torch is required for E14, but import remains tolerant for clarity.
    import torch
except ImportError as _exc:  # pragma: no cover - exercised only on torch-less envs.
    raise ImportError(
        "torch is required for E14; install via `pixi add pytorch` "
        "(or `pip install torch`)."
    ) from _exc


__all__ = ["E14Result", "run_e14"]
...
@dataclass
class E14Result:
    """Aggregate metrics returned by run_e14."""
...
def _per_class_centroid_init(
    ...
    Used to seed the torch trainer's prototype tower so step 0 starts
    with semantically sensible per-state centres rather than a tiny
    random init. Empty classes default to a small jitter draw.
    """
    ...
    for s in range(n_states):
        ...
        # Soft-rescale to ||mu|| < 0.5 so the Poincare projection in the
        # trainer's constructor is a near-no-op.
...
def run_e14(
    *,
    ...
    """Run torch-native end-to-end gradient training on ListOps.
    ...
    if fsm.vertex_ids[0] != "START":
        raise ValueError(
            f"E14 expects fsm.vertex_ids[0] == 'START'; "
            f"got {fsm.vertex_ids[0]!r}"
        )
    if fsm.vertex_ids[-1] != "ACCEPT":
        raise ValueError(
            f"E14 expects fsm.vertex_ids[-1] == 'ACCEPT'; "
    ...
    if 2 + derived_max_depth * 3 != n_states:
        raise ValueError(
            f"E14 cannot derive max_depth from {n_states} states; "
    ...
    # ------------------------------------------------------------------
    # Step 1 -- Dataset (ListOps walker, 16-dim one-hot tokens).
    # ------------------------------------------------------------------
    ...
    #
    # Substrate-independent: forward-backward expected_counts on the
    # observed integer state path, then bayesian_m_step_beta with a
    # uniform Beta prior. Phase A's Hamming distance is reported BEFORE
    # any torch update touches the posterior.
    # ------------------------------------------------------------------
    ...
    #
    # Encoder: FrozenEncoderTorch (input_dim=output_dim=16, hidden=32),
    # frozen by construction. Readout: TypedReadoutTorch with one head;
    # parameters become trainable members of the trainer via the
    # nn.ModuleDict registration in TorchEnergyTrainer.__init__.
    # ------------------------------------------------------------------
    ...
    readout = TypedReadoutTorch(
        ...
        n_epochs=1,  # We re-train the heads through the trainer's optimiser.
        lr=1e-3,
        seed=int(seed),
    )

    # Pre-compute encoded train features once (encoder is frozen).
    ...
    #
    # Per-epoch full-batch loop over the train split; every step does
    # forward + backward + optimizer.step on the trainer's owned Adam.
    # ------------------------------------------------------------------
    ...
    for _epoch in range(int(n_epochs)):
        # Shuffle index permutation per epoch for stochastic gradient noise.
        perm = rng.permutation(n_train)
        # Process the epoch as a single mini-batch -- n_train is small
        # (~few hundred) and full-batch keeps the loss curve smooth.
    ...
    # ------------------------------------------------------------------
    # Step 6 -- Phase C: held-out evaluation, axis quantisation, decision
    # trace, sigma/margin AUROC.
    # ------------------------------------------------------------------
    ...
    # Calibrate axis quantisers from a held-out PASS-1 over the test
    # split using the trained model. We pre-walk the test split to
    # collect sigma/energy/margin samples before the writer pass so the
    # axis nodes are stable for product-graph keys.
    ...
    # Empirical-distribution windows seeded from the train split (so the
    # KL-surprise channel has informative priors before we touch test).
    ...
    experiment_label = "E14"
    ablation_label = run_id.split("_")[1]
    sample_ids = [s.sample_id for s in test_ds.samples]

    # Per-sample arrays used by failure_margin_auroc.
    sigmas_step: list[float] = []
    margins_step: list[float] = []
    errors_step: list[bool] = []
    # Per-sample current-state name, used to build the structural-
    # ambiguity ground-truth label for `structural_ambiguity_auroc`.
    # On ListOps the structurally-ambiguous states are
    # S{d}_after_operand (the parser must choose continue-list vs.
    # close); a substring check on the vertex name is sufficient and
    # avoids hard-coding the numeric depth bound.
    ...
    with (
        ...
        with torch.no_grad():
            for step_idx in range(n_test):
                ...
                # Live legality bias from the (post-training) torch
                # posterior, snapshotted into numpy for downstream
                # numpy-only atoms.
                ...
                # Predicted distribution: trainer's softmax over
                # -d_poincare + legality_bias[cs]. The same atom
                # produces the no-mask comparison by zeroing the bias.
                p_masked = trainer.predicted_distribution(
                    obs_t, current_state=cs_idx
                ).detach().cpu().numpy().astype(np.float64)

                # No-mask: recompute logits without the legality-bias term.
                ...
                # Energy at the predicted prototype for diagnostic / trace.
                ...
                # Append the OBSERVED next-state to the empirical window
                # (window is read before append, so no leakage).
                ...
                # Per-sample arrays for failure_margin_auroc.
        ...
        # AUROC requires at least one positive AND one negative; guard.
        ...
        # Ground-truth label: 1 iff the sample's current state is one of
        # the grammar's structurally-ambiguous states. On ListOps that
        # is exactly S{d}_after_operand for d in {1,2,3} -- the only
        # states where the parser has a real continue-vs.-close choice.
        # We use a substring check on the vertex name so the metric is
        # robust to depth changes (S4_after_operand at deeper grammars
        # would also be picked up without code changes).
        ...
        # Margin: lower margin = more uncertain => map to higher
        # predicted-ambiguity score by inverting (1 - margin), so
        # bigger score means "more likely ambiguous". Sigma already
        # has the right sign convention.
        ...
        metric_pairs: list[tuple[str, float]] = [
            ...
            ("mean_kl_surprise", float(np.mean(kl_surprises_step)) if kl_surprises_step else 0.0),
