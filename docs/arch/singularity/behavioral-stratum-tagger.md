...
- Boolean `transition_legal` from [Graph Legality Mask](../graph/graph-legality-mask.md).
- Recent history slice (last $H$ predicted states) for loop-risk detection.
- Optional [Stabilizer Signature](../group/stabilizer-signature.md) (Phase 4); when absent, the `stabilizer-jump` rule is a no-op.
...
- **See also:** [Singularity Types Catalog](singularity-types.md), [Singularity Detector σ(x)](singularity-detector.md), [Stratified Partition Function](../energy/stratified-partition-function.md)
...
- **Driven by:** [Typed Score Record](../../drivers/typed-score-record.md), [Graph Legality Mask](../graph/graph-legality-mask.md)
- **Math:** [Mathematics.md §Whitney Stratification](../../Mathematics.md#whitney-stratification-and-group-action-orbit-types) (the geometric notion this atom deliberately does not implement; see [Stratified Partition Function](../energy/stratified-partition-function.md) for the Phase 5 atom that does).
