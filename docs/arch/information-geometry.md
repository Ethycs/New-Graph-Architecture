...
- **`kl_surprise(prev_mean, new_mean)`**: scalar surprise signal consumed by [Singularity Detector](./singularity-detector.md).
...
- **See also:** [Posterior Mask](./posterior-mask.md), [Hyperbolic Embedding](./hyperbolic-embedding.md), [Singularity Detector](./singularity-detector.md).
- **Drives:** the natural-gradient learning rates in [Energy Minimisation Trainer](./energy-minimization-trainer.md); the σ_kl_surprise component; the `crb_satisfied_fraction` reporting in Phase 7 runners.
