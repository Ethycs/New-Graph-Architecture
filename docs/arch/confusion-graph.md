...
- **Output:** (to analysis and [Graph FSM](./graph-fsm.md) refinement) directed multigraph $\mathcal{C} = (V, E_{\text{err}}, w_{\text{err}})$ where $w_{\text{err}}(q, q')$ is the confusion count or rate, plus optional low-margin edge set $E_{\text{near}}$.
...
## Links
- **See also:** [Margin Uncertainty](./margin-uncertainty.md), [Typed Score Record](./typed-score-record.md), [Graph FSM](./graph-fsm.md)
